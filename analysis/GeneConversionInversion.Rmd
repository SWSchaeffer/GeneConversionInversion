...
## 12. NG Gene Conversion Coverage
Purpose: This program estimates mean gene conversion coverage in coding sequences (CDS), corresponding intron sequences, and intergenic regions.

Source: NG Gene Conversion Coverage.f

Input:
1. At the console, Gene Arrangement Name (arr), where arr is the two letter inversion code [AR ST PP CH TL]
2. “GC_arr_MinMax.tsv”, a tab-separated value file with beginning and end coordinates of the 14 regions in the arr. Columns [Region, Beginning Coordinate, End Coordinate]
3. “GC_arr_Tracts.tsv”, from step 6 above, a tab separated value file with a list of gene conversion tracts, one tract per row.
4. “GC_arr_Transcript_Exons_List.tsv”, a tab-separated value file with information on each exon for each transcript.  Columns [Transcript, Exon, Beg, End, Outlier status either 1 non-outlier or 2 outlier].
5. “GC_arr_Transcript_Exons_Sort_List.tsv”, a tab-separated value file with information on each exon for each transcript.  The list is sorted so that the exons of each transcript are sequential.  This is done to take into account that some transcripts are embedded in the introns of other genes.  The data in this file is used to output coordinates of introns.  Columns [Transcript, Exon, Beg, End, Outlier Status either 1 non-outlier or 2 outlier].
6. “GC_arr_Transcripts_CDS_List.tsv”, a tab-separated value file with the coordinates for each transcript.  Columns [Transcript, Beg, End].

Output Files:
1. “GC_arr_Coverage_Summary_Stats.tsv”, a text file with summary statistics for gene conversion coverage in the 14 syntenic block regions. First row has column labels Columns [Region, Mean, Variance, Standard Deviation, Mean – 2SD, Mean +2SD]
2. “GC_arr_CDS_Coverage.tsv”, a tab-separated value file that estimates mean coverage across all exons of a gene.  This is done by adding up the coverage values over all exon sites and dividing them by the total number of nucleotides.  First row has column headers.  Columns [Transcript, Beginning Coordinate, End Coordinate, Region, Number of Nucleotides, Mean Coverage, Outlier Status either 1 non-outlier or 2 outlier]
3. “GC_arr_Intron_List.tsv”, a tab-separated value file that lists the intron coordinates for all transcripts.  The exon coordinates from input file v above are used.  Columns [Transcript, Intron number, Beginning Coordinate, End Coordinate, Outlier Status either 1 non-outlier or 2 outlier]
4. “GC_arr_Intron_Coverage.tsv”, a tab-separated value file that estimates mean coverage across all introns of a gene.  This is done by adding up the coverage values over all intron sites and dividing them by the total number of nucleotides.  The intron coordinates from output file iii above are used.  First row has column headers.  Columns [Transcript, Beginning Coordinate, End Coordinate, Region, Number of Nucleotides, Mean Coverage, Outlier Status either 1 non-outlier or 2 outlier]
5. “GC_arr_Intergenic_List.tsv”, a tab-separated value file that lists the intron coordinates for all transcripts.  The exon coordinates from input file vi above are used.  These coordinates are from the end of the last CDS to the beginning of the next CDS.  Embedded genes within introns are not used because they are contained within introns.  Two genes where the end and beginning coordinates overlap lack an intergenic region.  CDSs that overlap extend the Columns [Intergenic Region Name, Beginning Coordinate, End Coordinate]
6. “GC_arr_Intergenic_Coverage.tsv”, a tab-separated value file that estimates mean coverage of intergenic regions between CDSs.  This is done by adding up the coverage values over all intergenic sites and dividing them by the total number of nucleotides.  The intergenic coordinates from output file v above are used.  First row has column headers.  Columns [Intergenic Region Name, Beginning Coordinate, End Coordinate, Region, Number of Nucleotides, Mean Coverage]


## 13. NG Reg Don Rec Permute
...
## 14. NG GC Tract Length Region Permutation
...
## 15. NG Gene Conversion Permutation Test
...
## 16.	NG Gene Conversion Shuffle Tracts
...
## 17.	NG Gene Conversion Shuffle Tracts ML
